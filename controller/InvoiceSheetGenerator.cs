...
namespace TimeSheetTool
{
    ...
    public class SheetGenerator
    {
        ...
        {
            var projectsCollection = Properties.Settings.Default.Projects;
            foreach (string value in projectsCollection)
            {
                this.projectNames.Add(value);
                System.Console.WriteLine(string.Format("Retrieved project Name: {0}", value));
            }

        }

        public void GenerateInvoiceSheet(DateTime from, DateTime to, string fileName, string targetSheetName)
        {
            ...
            SpreadSheetUtil spreadSheetUtil = new SpreadSheetUtil(fileName, targetSheetName);

            // reads daily time sheet data from time sheet database
            List<TimeSheetEntry> timeSheetEntries = databaseUtil.RetrieveDailyDataForUsers(from, to, projectNames);
